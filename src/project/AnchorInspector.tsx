...
import { Check, Copy, Plus, Redo2, Trash2, Undo2, X } from 'lucide-react';
  ...
  return (
    <div className="grid gap-2">
      <dl className="grid gap-1 text-[10px] leading-4">
        ...
        <div className="grid grid-cols-[62px_minmax(0,1fr)] gap-2"><dt className="text-muted-foreground">World position</dt><dd className="font-semibold tabular-nums [overflow-wrap:anywhere]">{props.position}</dd></div>
        <div className="grid grid-cols-[62px_minmax(0,1fr)] gap-2"><dt className="text-muted-foreground">World XYZ °</dt><dd className="font-semibold tabular-nums [overflow-wrap:anywhere]">{props.rotation}</dd></div>
      </dl>
      {props.onEdit && <Button size="sm" variant="outline" onClick={props.onEdit}>Edit Anchor</Button>}
      {props.onCopy && <Button size="sm" variant="outline" onClick={props.onCopy}><Copy /> Copy Anchor</Button>}
    </div>
  );
}
