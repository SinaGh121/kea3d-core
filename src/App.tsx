...
function SceneTreeItem({ depth, node, onSelect, onInspect, onVisibility, selectedIds, primarySelectedId }: {
  depth: number;
  node: SceneNode;
  ...
  return (
    <div>
      ...
      {expanded && node.children.map((child) => (
        <SceneTreeItem key={child.id} depth={depth + 1} node={child} onSelect={onSelect} onInspect={onInspect} onVisibility={onVisibility} selectedIds={selectedIds} primarySelectedId={primarySelectedId} />
      ))}
    </div>
  );
}
            ...
            >
                <Popover open={sceneDetailsOpen || anchorEditing} onOpenChange={setSceneDetailsOpen}>
                  <PopoverTrigger asChild>
                    <Button aria-label="Selection details" variant="outline" className="w-full shrink-0 justify-start truncate" disabled={!selectionInfo} title={selectedName ?? undefined}><Info /><span className="truncate">{selectedName || 'Select an object to inspect'}</span></Button>
                  </PopoverTrigger>
                ...
                <Popover open={sceneControlsOpen} onOpenChange={setSceneControlsOpen}>
                  <PopoverTrigger asChild><Button variant="outline" className="shrink-0"><Settings2 /> Scene controls · Anchors · Explode</Button></PopoverTrigger>
                ...
                <ScrollArea className="min-h-0 flex-1 pr-2">
                  {sceneTree.map((node) => (
                    <SceneTreeItem key={node.id} depth={0} node={node} onSelect={selectObject} onInspect={() => inspectRequestRef.current()} onVisibility={toggleObjectVisibility} selectedIds={selectedIdSet} primarySelectedId={selectedId} />
                  ))}
                </ScrollArea>
            </ResponsivePanel>
          )}
