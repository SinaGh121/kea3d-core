...
import { Axis3D, BetweenHorizontalStart, Box, Boxes, Camera, Check, ChevronDown, ChevronRight, ChevronUp, Copy, Download, Eye, EyeOff, FileBox, FlipHorizontal2, Focus, FolderOpen, Grid3X3, Info, Keyboard, Layers3, Maximize2, Move3D, Network, Orbit, PaintBucket, Pause, Play, Redo2, Repeat2, RotateCcw, Ruler, Scan, ScanBox, ScissorsLineDashed, Settings2, Share2, Sun, Undo2, Upload, View, X } from 'lucide-react';
...
import { ViewerToolbar, type ViewerAction } from '@/components/ViewerToolbar';
...
const acceptedExtensionValues = ['kea3dp', 'kea3d', 'glb', 'gltf', 'stl', '3mf', 'obj', 'mtl', 'ply', 'fbx', 'dae', 'step', 'stp', 'iges', 'igs', 'brep', 'bin', 'png', 'jpg', 'jpeg', 'webp', 'avif', 'ktx2'];
...
const legalDocuments = {
  ...
  thirdParty: {
    ...
    load: () => Promise.all([
      ...
      import('../THIRD_PARTY_LICENSES.txt?raw'),
      import('../legal/evidence/scroll-LICENSE.txt?raw'),
      import('../legal/evidence/assimp-bundled-notices.txt?raw'),
      import('../legal/evidence/basis-LICENSES-Apache-2.0.txt?raw'),
      import('../legal/evidence/basis-LICENSES-BSD-3-clause.txt?raw'),
      import('../legal/evidence/basis-LICENSES-MIT.txt?raw'),
      import('../legal/evidence/basis-LICENSES-Zlib.txt?raw'),
      import('../legal/evidence/basis-zstd-LICENSE?raw'),
    ]).then((documents) => documents.map(({ default: content }) => content).join('\n\n')),
  },
} as const;
...
function ResponsivePanel({ title, description, onClose, children, footer, desktopClassName, compactFixedHeight, compactMode = 'sheet', compactHeight = compactWorkspaceHeight, compactWorkspaceExpanded = false, onCompactWorkspaceExpandedChange, contentClassName, titleClassName }: {
  ...
  const sideWorkspace = useSideWorkspace();
  const panelRef = useRef<HTMLElement>(null);
  useEffect(() => {
    if (!compact || compactMode !== 'workspace') return;
    const previous = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    panelRef.current?.focus({ preventScroll: true });
    return () => {
      if (previous?.isConnected) previous.focus({ preventScroll: true });
      else document.querySelector<HTMLButtonElement>('[aria-label="More tools"]')?.focus({ preventScroll: true });
    };
  }, [compact, compactMode]);
    ...
    if (compactMode === 'workspace') {
      return (
        <aside
          ...
          tabIndex={-1}
          aria-label={title}
          className={cn('kea-workspace absolute bottom-0 z-40 flex min-h-0 flex-col gap-0 overflow-hidden border bg-popover text-sm text-popover-foreground shadow-2xl outline-none', sideWorkspace ? 'top-0 right-0 rounded-l-xl' : 'inset-x-0 rounded-t-2xl')}
          style={sideWorkspace ? { width: sideWorkspaceWidth } : { height: compactWorkspaceExpanded ? compactWorkspaceExpandedHeight : compactHeight }}
        >
          ...
          <div data-testid="responsive-panel-scroll" className={cn('min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain px-4 pb-3 max-lg:[&_[data-slot=button]]:min-h-11 max-lg:[&_[data-slot=button]]:min-w-11', contentClassName)}>{children}</div>
          {footer && <div className="shrink-0 border-t px-4 py-3 [&_button]:min-h-11">{footer}</div>}
        </aside>
      );
    }
    ...
    return (
      <Sheet open onOpenChange={(open) => { if (!open) onClose(); }}>
        ...
        >
          ...
          {footer && <div className="shrink-0 border-t px-4 pt-3 [&_button]:min-h-11">{footer}</div>}
        </SheetContent>
      </Sheet>
    );
  }
  ...
  return (
    <Card size="sm" className={cn(desktopClassName, 'max-h-[calc(100%_-_100px)] min-h-0 overflow-hidden')}>
...
function SceneTreeItem({ depth, node, onSelect, onInspect, onVisibility, selectedIds, primarySelectedId, forceExpanded = false, revealRequest = 0 }: {
  ...
  const showChildren = expanded || forceExpanded;
    ...
    <div>
      ...
      >
        {node.children.length > 0 ? (
          <Button variant="ghost" size="icon-xs" disabled={forceExpanded} aria-expanded={showChildren} aria-label={showChildren ? `Collapse ${node.name}` : `Expand ${node.name}`} onClick={() => setExpanded(!expanded)}>
        ...
        <button
          ...
          onDoubleClick={() => { onSelect(node.id, { additive: false, range: false }); onInspect(); }}
      ...
      {showChildren && node.children.map((child) => (
        <SceneTreeItem key={child.id} depth={depth + 1} node={child} onSelect={onSelect} onInspect={onInspect} onVisibility={onVisibility} selectedIds={selectedIds} primarySelectedId={primarySelectedId} forceExpanded={forceExpanded} revealRequest={revealRequest} />
      ))}
    </div>
  );
}
...
export default function App() {
  const inspectRequestRef = useRef<() => void>(() => undefined);
  const nativeShell = isNativeShell();
  const releaseUpdates = useReleaseUpdates(nativeShell);
  ...
  const discardRequestRef = useRef<{ promise: Promise<boolean>; resolve: (value: boolean) => void } | null>(null);
  ...
  const [nativeReadCancellable, setNativeReadCancellable] = useState(false);
  ...
  const [sceneRevealRequest, setSceneRevealRequest] = useState(0);
  const [sceneFilter, setSceneFilter] = useState<SceneFilter>('all');
  const sceneResults = useMemo(() => searchScene(sceneTree, sceneQuery, sceneFilter), [sceneTree, sceneQuery, sceneFilter]);
  const sceneFiltering = sceneQuery.trim() !== '' || sceneFilter !== 'all';
  ...
  const compactLayerOpen = compactLayout && (
    mobileToolsVisible || settingsVisible || infoVisible || treeVisible
  ...
  const hasUnsavedChanges = useCallback(() => {
    const viewer = viewerRef.current;
    return Boolean(viewer && savedCheckpointRef.current &&
      (viewer.getDocumentCheckpoint() !== savedCheckpointRef.current || viewer.hasMaterialPreview()));
  }, []);
  const confirmDiscard = useCallback((): Promise<boolean> => {
    if (!hasUnsavedChanges()) return Promise.resolve(true);
    if (discardRequestRef.current) return discardRequestRef.current.promise;
  ...
  const finishDiscardPrompt = (discard: boolean) => {
    discardRequestRef.current?.resolve(discard);
  ...
  useEffect(() => {
    ...
    void import('@tauri-apps/api/window').then(async ({ getCurrentWindow }) => {
      ...
      const stop = await current.onCloseRequested(async event => {
        ...
        event.preventDefault();
        if (await confirmDiscard()) await current.destroy();
      });
      if (disposed) stop(); else unlisten = stop;
    }).catch(() => undefined);
    return () => { disposed = true; unlisten?.(); };
  }, [desktopNativeShell, hasUnsavedChanges, confirmDiscard]);
  const loadActive = Boolean(progress);
  useEffect(() => {
    if (loadStartedAt === null || !loadActive) return;
    const update = () => setLoadElapsed(Math.floor((Date.now() - loadStartedAt) / 1000));
    update();
    const timer = window.setInterval(update, 1000);
    return () => window.clearInterval(timer);
  }, [loadStartedAt, loadActive]);

  const loadFiles = useCallback(async (fileList: FileList | File[], discardApproved = false) => {
    const files = Array.from(fileList);
    const viewerPromise = viewerPromiseRef.current;
    if (!viewerPromise || files.length === 0) return;
    if (!discardApproved && !await confirmDiscard()) {
    ...
    try {
      ...
      savedCheckpointRef.current = viewer.getDocumentCheckpoint();
      setSceneQuery('');
      setSceneFilter('all');
      ...
      if (!isLoadCancellation(loadError) && loadAbortRef.current === controller) {
        if (isProjectResourceRecoveryError(loadError)) {
          ...
          const message = loadError instanceof Error ? loadError.message : 'The model could not be opened.';
          // Native synthetic files can own transferred buffers: ask the picker again for those.
          retryFilesRef.current = files.every(file => file.size > 0) ? files : null;
  ...
  }, [compactLayout, displayMode, gridVisible, projection, rotationMode, confirmDiscard]);
      ...
      nativeOpenQueueRef.current = nativeOpenQueueRef.current
        ...
        .then(async () => {
          ...
          if (!await confirmDiscard()) {
            await Promise.all(pending.map(entry => invoke('finish_pending_open_file', { id: entry.id }).catch(() => undefined)));
            return;
          }
          setLoadStartedAt(Date.now());
          setLoadElapsed(0);
          setLoadFailure(null);
          setLoadingName(pending.find((entry) => /\.(kea3dp|kea3d|glb|gltf|stl|3mf|obj|ply|fbx|dae|step|stp|iges|igs|brep)$/i.test(entry.name))?.name ?? 'Local model');
          ...
          if (nativeCadEntry) {
            ...
            try {
              ...
              await loadFiles([nativeImport.file], true);
            ...
            for (const entry of pending) {
              ...
              if (entry.requiresStreaming && entry.sourceUrl && window.kea3dNativeFile) {
                ...
                try {
                  ...
                  setNativeReadCancellable(false);
  ...
  }, [loadFiles, nativeShell, confirmDiscard]);
  ...
  const toggleMaterialPanel = () => {
    if (!materialVisible && !selectionInfo?.meshes && !materialEditState.previewActive) {
      toast.info('Select a mesh to set its material. Anchors do not have surface materials.');
      return;
    }
    if (materialVisible) {
      setMaterialVisible(false);
  ...
  const toggleSettingsPanel = () => {
    ...
    if (next) {
      ...
      setSectionVisible(false);
      setLightingVisible(false);
      if (materialVisible) setMaterialVisible(false);
      closeMeasurement();
    }
  };
  ...
  useEffect(() => {
    inspectRequestRef.current = () => { if (!infoVisible) toggleInfoPanel(); };
  });
  ...
  const toggleAnimationPanel = () => {
    ...
    if (next) { if (compactLayout) { setTreeVisible(false); setInfoVisible(false); } setAdjustVisible(false); setExportVisible(false); setSectionVisible(false); setSettingsVisible(false); setLightingVisible(false); if (materialVisible) setMaterialVisible(false); closeMeasurement(); }
  };
  const toggleSectionPanel = () => {
    const next = !sectionVisible;
    setSectionVisible(next);
    if (next) { if (compactLayout) { setTreeVisible(false); setInfoVisible(false); } setAdjustVisible(false); setExportVisible(false); setAnimationVisible(false); setSettingsVisible(false); setLightingVisible(false); if (materialVisible) setMaterialVisible(false); closeMeasurement(); }
  };
  ...
  const selectObject = (objectId: string, options: { additive: boolean; range: boolean }) => {
    if (options.range && selectionAnchorRef.current) {
      const orderedIds = flattenSceneNodeIds(sceneFiltering ? sceneResults.nodes : sceneTree);
  ...
  const exportCorrectedGlb = async () => {
    ...
    setExporting(true);
    const exportingViewer = viewerRef.current;
    const checkpoint = exportingViewer.getDocumentCheckpoint();
    try {
  ...
  useEffect(() => {
    const handleKeyDown = (event: globalThis.KeyboardEvent) => {
      ...
      if ((event.defaultPrevented && key !== 'escape') || (key === 'escape' && target instanceof Element && target.closest('[data-slot="popover-content"]'))) return;
      if (key === 'escape' && (sceneDetailsOpen || sceneControlsOpen || anchorEditing)) {
        event.preventDefault();
      ...
      if (key === 'escape' && compactLayout && compactLayerOpen) {
        ...
        else if (materialVisible) setMaterialVisible(false);
      ...
      if (key === 'escape' && modelInfo) {
        ...
        if (viewSelectorVisible) {
          viewerRef.current?.setViewSelectorVisible(false);
          return;
        }
  ...
  const anchorPosition = primaryAnchor ? primaryAnchor.position.map((value, index) => `${'XYZ'[index]} ${formatMetricLength(value, displayUnit)}`).join(' · ') : '';
  const anchorOrientation = primaryAnchor ? primaryAnchor.worldRotation.map((value, index) => `${'XYZ'[index]} ${formatDimension(value)}°`).join(' · ') : '';
  const anchorRows: Array<[string, string]> = primaryAnchor ? [
    ['Name', primaryAnchor.name], ['ID', primaryAnchor.id], ['Parent', primaryAnchor.parentName ?? 'Model root'],
    ['World position', anchorPosition], ['World XYZ °', anchorOrientation],
    ...(selectionInfo!.anchors!.length > 1 ? [['Anchors', `${selectionInfo!.anchors!.length} selected · showing last selected`] as [string, string]] : []),
  ] : [];
  const copyAnchorInfo = async () => {
    if (!primaryAnchor) return;
    try {
      await navigator.clipboard.writeText(anchorRows.map(([label, value]) => `${label}: ${value}`).join('\n'));
      toast.success('Anchor information copied');
    } catch { toast.error('Clipboard access failed. You can select and copy the values from Anchor info.'); }
  };
  ...
  const viewerActions: ViewerAction[] = [
    { id: 'fit', label: 'Fit model', group: 'Navigate', priority: 0, icon: <Focus />, shortcut: 'F', run: () => viewerRef.current?.fit() },
    { id: 'views', label: 'View selector', group: 'Navigate', priority: 1, icon: <View />, active: viewSelectorVisible, run: toggleViewSelector },
    { id: 'objects', label: 'Scene objects', group: 'Inspect', priority: 2, icon: <Layers3 />, active: treeVisible, run: toggleScenePanel },
    { id: 'isolate', label: isolateSelectionLabel, shortLabel: isolationActive ? 'Exit isolate' : 'Isolate', group: 'Inspect', priority: 3, icon: <Scan />, active: isolationActive, disabledReason: !canIsolateSelection && !isolationActive ? 'Select a mesh to isolate' : undefined, run: toggleSelectionIsolation },
    { id: 'anchors', label: anchorsVisible ? 'Hide anchors' : 'Show anchors', shortLabel: 'Anchors', group: 'Inspect', priority: 4, icon: <Axis3D />, available: anchorCount > 0, active: anchorsVisible, run: () => toggleAnchorsVisible(!anchorsVisible) },
    { id: 'display', label: `Display: ${displayMode[0].toUpperCase()}${displayMode.slice(1)} · click to cycle`, shortLabel: `Display: ${displayMode}`, group: 'Appearance', priority: 5, icon: <Network />, active: displayMode !== 'solid', run: cycleDisplayMode },
    { id: 'projection', label: 'Orthographic', group: 'Navigate', priority: 6, icon: <ScanBox />, active: projection === 'orthographic', run: toggleProjection },
    { id: 'orbit', label: 'Free orbit', group: 'Navigate', priority: 7, icon: <Orbit />, active: rotationMode === 'free', run: toggleRotationMode },
    { id: 'grid', label: 'Grid', group: 'Appearance', priority: 8, icon: <Grid3X3 />, active: gridVisible, run: toggleGrid },
    { id: 'lighting', label: 'Lighting', group: 'Appearance', priority: 9, icon: <Sun />, active: lightingVisible, run: toggleLightingPanel },
    { id: 'info', label: 'Model information', shortLabel: 'Info', group: 'Inspect', priority: 10, icon: <Info />, active: infoVisible, run: toggleInfoPanel },
    { id: 'measure', label: 'Measure distance', shortLabel: 'Measure', group: 'Inspect', priority: 11, icon: <BetweenHorizontalStart />, active: measurementVisible, run: toggleMeasurementPanel },
    ...
    { id: 'fullscreen', label: 'Fullscreen', group: 'Application', priority: 19, icon: <Maximize2 />, run: () => void toggleFullscreen() },
    { id: 'settings', label: releaseUpdates.status === 'available' ? 'Settings · Update available' : 'Settings', group: 'Application', priority: 20, icon: <span className="relative"><Settings2 />{releaseUpdates.status === 'available' && <span className="absolute -right-1 -top-1 size-2 rounded-full bg-primary ring-2 ring-background" />}</span>, active: settingsVisible, run: toggleSettingsPanel },
  ];
    ...
    <TooltipProvider>
      <main className="relative h-full w-full min-w-[280px] bg-background text-foreground">
        ...
        >
          <div
            ...
            style={{ bottom: hasSideWorkspace ? 0 : compactViewportBottom, right: hasSideWorkspace ? sideWorkspaceWidth : 0 }}
          />
          ...
          {!modelInfo && (
            <Button variant="outline" size="icon-lg" type="button" className="absolute top-5 right-5 z-20 rounded-xl bg-card/85 shadow-lg backdrop-blur-md max-lg:top-3 max-lg:right-3" aria-label="Settings" onClick={toggleSettingsPanel}>
              <Settings2 />{releaseUpdates.status === 'available' && <span className="absolute right-1 top-1 size-2 rounded-full bg-primary ring-2 ring-background" aria-label="Update available" />}
            </Button>
          )}
          ...
          {modelInfo && materialEditState.previewActive && !materialVisible && (
            <div role="status" style={hasSideWorkspace ? { maxWidth: `calc(100% - ${sideWorkspaceWidth} - 24px)` } : undefined} className="absolute top-16 left-3 z-45 flex max-w-[calc(100%_-_24px)] flex-wrap items-center gap-1 rounded-lg border bg-card p-2 text-xs shadow-lg lg:top-auto lg:right-[340px] lg:bottom-5 lg:left-[340px] max-lg:[&_button]:min-h-11">
              <span>Material preview · not applied</span>
              <Button size="sm" variant="outline" onClick={toggleMaterialPanel}>Resume</Button>
              <Button size="sm" variant="outline" onClick={closeMaterialPanel}>Discard</Button>
              <Button size="sm" onClick={applyMaterial}>Apply</Button>
            </div>
          )}
          ...
          {!modelInfo && !progress && nativeStartupResolved && (
            <section aria-label="Open a model" className="absolute top-1/2 left-1/2 z-10 flex w-[min(410px,calc(100%_-_40px))] -translate-x-1/2 -translate-y-1/2 flex-col items-center text-center">
              ...
              <small className="mt-4 max-w-sm text-[11px] leading-relaxed text-muted-foreground">KEA3DP · KEA3D · STEP · IGES · GLB · STL · 3MF · OBJ · PLY · FBX · DAE</small>
            ...
            >
              <Accordion type="multiple" defaultValue={['appearance', 'viewer']}>
                ...
                <AccordionItem value="about">
                  ...
                  <AccordionContent>
                    <Suspense fallback={null}><AboutContent onOpenExternal={openExternalLink} onOpenLegal={openLegalDocument} updates={nativeShell ? releaseUpdates : undefined} /></Suspense>
                  </AccordionContent>
                </AccordionItem>
              </Accordion>
            </ResponsivePanel>
          ...
          {modelInfo && infoVisible && (
            <ResponsivePanel
              title={primaryAnchor && selectionInfo?.meshes === 0 ? 'Anchor info' : selectionInfo ? 'Selected info' : 'Model info'}
            ...
            >
              <Suspense fallback={null}><AnchorInspector mode="rows" rows={primaryAnchor ? [...anchorRows, ...(selectionInfo?.meshes ? infoRows : [])] : infoRows} /></Suspense>
              {primaryAnchor && <div className="mt-2 grid grid-cols-2 gap-2"><Button variant="outline" size="sm" onClick={() => viewerRef.current?.fitSelection()}><Focus /> Fit Anchor</Button><Button variant="outline" size="sm" onClick={() => void copyAnchorInfo()}><Copy /> Copy Anchor</Button></div>}
            </ResponsivePanel>
          )}
          ...
          {modelInfo && treeVisible && (
            ...
            >
                <div className="flex shrink-0 gap-2">
                  <Input aria-label="Search scene objects" placeholder="Search objects…" value={sceneQuery} onChange={event => setSceneQuery(event.target.value)} className="min-h-11 min-w-0 flex-1" />
                  <select aria-label="Filter scene objects" value={sceneFilter} onChange={event => setSceneFilter(event.target.value as SceneFilter)} className="min-h-11 max-w-24 rounded-lg border bg-background px-2 text-xs">
                    <option value="all">All</option><option value="hidden">Hidden</option><option value="anchors">Anchors</option>
                  </select>
                </div>
                <span role="status" className="sr-only">{sceneResults.matches} {sceneFiltering ? 'matches' : 'objects'}</span>
                <div className="flex shrink-0 gap-1">
                <Popover open={sceneDetailsOpen || anchorEditing} onOpenChange={setSceneDetailsOpen}>
                  <PopoverTrigger asChild>
                    <Button aria-label="Selection details" variant="outline" className="min-w-0 flex-1 justify-start truncate" disabled={!selectionInfo} title={selectedName ?? undefined}><Info /><span className="truncate">{selectedName || 'Selection'}</span></Button>
                  </PopoverTrigger>
                  <PopoverContent aria-label="Selection details" side="right" className="max-h-[min(70dvh,460px)] w-[min(310px,90vw)] overflow-y-auto">
                ...
                <Popover open={sceneControlsOpen} onOpenChange={setSceneControlsOpen}>
                  <PopoverTrigger asChild><Button variant="outline" size="icon-sm" aria-label="Scene controls · Anchors · Explode" className="shrink-0"><Settings2 /></Button></PopoverTrigger>
                  <PopoverContent aria-label="Scene controls" side="right" className="grid max-h-[70dvh] w-[min(310px,90vw)] gap-3 overflow-y-auto">
                <Suspense fallback={null}>
                  <AnchorInspector
                    mode="visibility"
                    count={anchorCount}
                    ...
                    onCreate={projectSession ? undefined : () => { setSceneControlsOpen(false); createAnchor(); }}
                ...
                <Button size="icon-sm" variant="outline" aria-label="Reveal selected" title="Reveal selected" disabled={!selectedId} onClick={() => { setSceneQuery(''); setSceneFilter('all'); setSceneRevealRequest(value => value + 1); }}><Focus /></Button>
                {sceneFiltering && <Button size="icon-sm" variant="ghost" aria-label="Clear scene filters" onClick={() => { setSceneQuery(''); setSceneFilter('all'); }}><X /></Button>}
                </div>
                <ScrollArea className="min-h-0 flex-1 pr-2">
                  {sceneResults.nodes.length === 0 && <p className="py-4 text-xs text-muted-foreground">No matching objects. Change the search or filter.</p>}
                  {sceneResults.nodes.map((node) => (
                    <SceneTreeItem key={node.id} depth={0} node={node} onSelect={selectObject} onInspect={() => inspectRequestRef.current()} onVisibility={toggleObjectVisibility} selectedIds={selectedIdSet} primarySelectedId={selectedId} forceExpanded={sceneFiltering} revealRequest={sceneRevealRequest} />
                  ))}
                </ScrollArea>
            </ResponsivePanel>
          )}
          ...
          {modelInfo && exportVisible && (
            <ResponsivePanel
              ...
              footer={<Button className="w-full" disabled={exporting} onClick={() => void exportCorrectedGlb()}><Download /> {exporting ? 'Exporting…' : projectSession ? 'Export flattened GLB' : 'Export GLB'}</Button>}
            >
              {projectSession && (
                <Suspense fallback={null}>
                  <ProjectSaveControls
          ...
          {loadFailure && !progress && <Card role="alert" className="absolute bottom-4 left-1/2 z-50 w-[min(420px,calc(100%_-_32px))] -translate-x-1/2 gap-3 p-4 shadow-2xl">
            <strong className="text-sm">Could not open model</strong>
            <p className="max-h-24 overflow-y-auto text-xs [overflow-wrap:anywhere]">{loadFailure}</p>
            <p className="text-xs text-muted-foreground">Try again, or choose the model together with its required companion files.</p>
            <div className="flex flex-wrap gap-2">
              {retryFilesRef.current && <Button size="sm" onClick={() => { const files = retryFilesRef.current; if (files) void loadFiles(files); }}>Retry</Button>}
              <Button size="sm" variant="outline" onClick={() => { setLoadFailure(null); retryFilesRef.current = null; void chooseModelFiles(); }}>Choose files</Button>
              <Button size="sm" variant="ghost" onClick={() => { setLoadFailure(null); retryFilesRef.current = null; }}>Dismiss</Button>
            </div>
          </Card>}
          {(progress || !nativeStartupResolved) && (
            <div className={cn(
              ...
              <Card className={cn(
                ...
                <Progress aria-label="Current loading stage" value={progress?.value === undefined ? null : progress.value * 100} />
                {progress && <p className="text-xs text-muted-foreground tabular-nums">{loadElapsed}s elapsed · {progress.value === undefined ? 'Working…' : `${Math.round(progress.value * 100)}% of this stage`}</p>}
                {progress && loadingNotice && <p className="text-[11px] leading-relaxed text-muted-foreground">{loadingNotice}</p>}
                {progress && (!readingNativeFile || nativeReadCancellable) && <Button variant="outline" size="sm" onClick={cancelLoad}><X /> Cancel</Button>}
              </Card>
            </div>
          )}
      ...
      <AlertDialog open={discardPrompt} onOpenChange={open => { if (!open) finishDiscardPrompt(false); }}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter><AlertDialogCancel onClick={() => finishDiscardPrompt(false)}>Keep editing</AlertDialogCancel><AlertDialogAction onClick={() => finishDiscardPrompt(true)}>Discard changes</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        >
          ...
          >
            <div className="p-4 pr-6">
              {(legalDocumentContent.match(/[\s\S]{1,12000}(?:\n|$)|[\s\S]{1,12000}/g) ?? []).map((text, index) => (
                <pre key={index} className="m-0 w-full min-w-0 max-w-full whitespace-pre-wrap font-mono text-[11px] leading-relaxed text-foreground [overflow-wrap:anywhere] [content-visibility:auto] [contain-intrinsic-size:auto_800px]">
                  {text}
                </pre>
              ))}
            </div>
