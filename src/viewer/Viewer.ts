import { anchorWorldRotationDegrees } from '../project/anchorPresentation';
import {
  AnimationMixer,
  AxesHelper,
  BackSide,
...
export class Viewer {
  ...
  constructor(
    ...
    private readonly onInspect: () => void = () => undefined,
  ) {
  ...
  fit(view?: CameraView): void {
    if (!this.currentModel) return;
    if (this.viewSelector.isVisible() && !view) {
      this.fitViewSelector();
      return;
    }
    if (view) {
  ...
  setViewSelectorVisible(visible: boolean, restorePrevious = true): boolean {
    ...
    if (next && this.currentModel) {
      this.viewSelectorPreviousCamera = this.getCameraState();
      this.viewSelectorPreviousLimits = { min: this.controls.minDistance, max: this.controls.maxDistance };
      this.viewSelector.configure(this.currentModel);
      this.fitViewSelector();
    }
    this.viewSelector.setVisible(next);
    if (!next) {
      const previous = this.viewSelectorPreviousCamera;
      const limits = this.viewSelectorPreviousLimits;
      this.viewSelectorPreviousCamera = null;
      this.viewSelectorPreviousLimits = null;
      if (restorePrevious && previous) {
        // Flush pending OrbitControls damping before restoring the exact camera.
  ...
  private fitViewSelector(): void {
    if (!this.currentModel) return;
    const bounds = this.viewSelector.getBounds().expandByObject(this.currentModel);
    const direction = this.camera.position.clone().sub(this.controls.target).normalize();
    this.frameBounds(direction, bounds, true);
    if (this.camera === this.orthographicCamera) {
      const aspect = this.container.clientWidth / Math.max(this.container.clientHeight, 1);
      this.orthographicHeight /= Math.min(aspect, 1);
      this.updateOrthographicFrustum(aspect);
    }
  }
  ...
  getDocumentCheckpoint(): string {
    if (!this.currentModel) return '';
    // View-only camera, lighting, visibility, explosion and selection are excluded.
    return JSON.stringify([this.commandHistory.getRevision(),
      this.currentModel.position.toArray(), this.currentModel.quaternion.toArray(),
      this.currentModel.scale.toArray()]);
  }

  hasMaterialPreview(): boolean { return this.materialPreview !== null; }
  ...
  private prepareAnchors(anchors: ComponentAnchor[]): void {
    ...
    anchors.forEach((anchor) => {
      ...
      // Concentric silhouettes keep the accent readable on light and dark surfaces.
      for (const [radius, color, order] of [[0.29, 0x101010, 10_097], [0.235, 0xffffff, 10_098]]) {
        const border = new Mesh(new SphereGeometry(radius, 16, 12), new MeshBasicMaterial({
          color, depthTest: false, depthWrite: false, toneMapped: false, transparent: true,
        }));
        border.renderOrder = order;
        border.userData.kea3dAnchorObjectId = anchor.object.uuid;
        marker.add(border);
      }
  ...
  private anchorInfo(anchor: ComponentAnchor): AnchorInfo {
    ...
    return {
      ...
      worldRotation: anchorWorldRotationDegrees({ rotation: rotation.toArray() }),
  ...
  private updateAnchorMarkers(): void {
    ...
    this.anchorMarkers.forEach((marker, source) => {
      ...
      marker.children.forEach((child) => child.scale.setScalar(Math.max(worldPerPixel * 24 * (selected && child instanceof Mesh ? 1.3 : 1), 0.000_001)));
      marker.visible = this.isEffectivelyVisible(source);
      const origin = marker.children.find((child) => child instanceof Mesh) as Mesh | undefined;
      if (origin?.material instanceof MeshBasicMaterial) origin.material.opacity = 1;
    });
  }
  ...
  private readonly handleDoubleClick = (event: MouseEvent): void => {
    if (event.button !== 0 || !this.currentModel || this.measurementEnabled || this.viewSelector.isVisible()) return;
    this.updatePointerRay(event);
    this.updateAnchorMarkers();
    const anchor = this.anchorsVisible ? pickAnchorMarker(this.raycaster, this.anchorMarkers) : undefined;
    const object = anchor ?? pickVisibleMesh(this.raycaster, this.currentModel)?.object;
    if (!object) return;
    this.selectObject(object.uuid);
    this.onInspect();
  };

  private updatePointerRay(event: Pick<MouseEvent, 'clientX' | 'clientY'>): void {
  ...
  private resize(): void {
    ...
    this.perspectiveCamera.updateProjectionMatrix();
    this.updateOrthographicFrustum(aspect);
    if (this.viewSelector.isVisible()) this.fitViewSelector();
  }
