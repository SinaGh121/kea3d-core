...
import { blendCompatibilityMessage, cadNoGeometryMessage, constrainedCadBytes, extremeFileBytes, isLoadCancellation, largeFileBytes, loadSizeNotice, throwIfLoadCancelled } from './loadControl';
...
describe('load control', () => {
  ...
  it('explains the reliable GLB conversion path for unsupported BLEND files', () => {
    const message = blendCompatibilityMessage();
    expect(message).toContain('files are not supported');
