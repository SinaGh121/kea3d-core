...
export async function loadModelFiles(
  ...
  const mainFile = files.find((file) => ['stl', '3mf', 'obj', 'ply', 'fbx', 'dae', 'step', 'stp', 'iges', 'igs', 'brep'].includes(fileExtension(file.name)));
  if (!mainFile) {
    if (files.some((file) => fileExtension(file.name) === 'blend')) throw new Error(blendCompatibilityMessage());
    throw new Error('Choose a supported 3D model file.');
  }
