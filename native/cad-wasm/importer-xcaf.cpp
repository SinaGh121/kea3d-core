// Based on occt-import-js 0.0.23 (LGPL-2.1), copyright Viktor Kovacs.
// Kea3D modification: resolve inherited XCAF surface styles once per mesh node.
...
typedef NCollection_DataMap<TopoDS_Shape, ResolvedFaceColor, TopTools_ShapeMapHasher> FaceColors;

static FaceColors CollectFaceColors (const TDF_Label& label)
{
    XCAFPrs_IndexedDataMapOfShapeStyle styles;
    XCAFPrs::CollectStyleSettings (label, TopLoc_Location (), styles);
    FaceColors colors;
    for (XCAFPrs_DataMapIteratorOfIndexedDataMapOfShapeStyle it (styles); it.More (); it.Next ()) {
        const XCAFPrs_Style& style = it.Value ();
        if (!style.IsVisible () || !style.IsSetColorSurf ()) continue;
        const TopoDS_Shape& shape = it.Key ();
        // TopAbs orders compounds before solids, shells, and faces: more specific wins.
        const int priority = static_cast<int> (shape.ShapeType ());
        const auto assign = [&](const TopoDS_Shape& face) {
            const ResolvedFaceColor value {style.GetColorSurf (), priority};
            if (!colors.IsBound (face)) colors.Bind (face, value);
            else if (priority >= colors.Find (face).priority) colors.ChangeFind (face) = value;
        };
        if (shape.ShapeType () == TopAbs_FACE) assign (shape);
        else for (TopExp_Explorer faces (shape, TopAbs_FACE); faces.More (); faces.Next ()) assign (faces.Current ());
    }
    return colors;
}
...
{
public:
    XcafFace (const TopoDS_Face& face, const Handle (XCAFDoc_ShapeTool)& shapeTool, const Handle (XCAFDoc_ColorTool)& colorTool, const FaceColors& colors) :
        OcctFace (face),
        shapeTool (shapeTool),
        colorTool (colorTool),
        colors (colors)
    ...
    virtual bool GetColor (Color& color) const override
    {
        if (colors.IsBound (face)) {
            const Quantity_Color& value = colors.Find (face).color;
            color = Color (value.Red (), value.Green (), value.Blue ());
            return true;
        }
...
{
public:
    XcafShapeMesh (const TopoDS_Shape& shape, const Handle (XCAFDoc_ShapeTool)& shapeTool, const Handle (XCAFDoc_ColorTool)& colorTool, const FaceColors& colors) :
        Mesh (),
        shape (shape),
        shapeTool (shapeTool),
        colorTool (colorTool),
    ...
    virtual void EnumerateFaces (const std::function<void (const Face& face)>& onFace) const override
    {
        for (TopExp_Explorer ex (shape, TopAbs_FACE); ex.More (); ex.Next ()) {
            ...
            XcafFace outputFace (face, shapeTool, colorTool, colors);
            onFace (outputFace);
        }
    }
...
{
public:
    XcafStandaloneFacesMesh (const TopoDS_Shape& shape, const Handle (XCAFDoc_ShapeTool)& shapeTool, const Handle (XCAFDoc_ColorTool)& colorTool, const FaceColors& colors) :
        Mesh (),
        shape (shape),
        shapeTool (shapeTool),
        colorTool (colorTool),
...
private:
    ...
    {
        const FaceColors colors = CollectFaceColors (label);
        // Enumerate solids
        for (TopExp_Explorer ex (shape, TopAbs_SOLID); ex.More (); ex.Next ()) {
            const TopoDS_Shape& currentShape = ex.Current ();
            XcafShapeMesh outputShapeMesh (currentShape, shapeTool, colorTool, colors);
        ...
        XcafStandaloneFacesMesh standaloneFacesMesh (shape, shapeTool, colorTool, colors);
