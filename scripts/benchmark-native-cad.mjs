...
function waitForExit(child) {
  // Small fixtures may finish while the process sampler is starting.
...
async function runOnce(file, worker, iteration, timeoutMs) {
  ...
  // Drain stdout before awaiting sampler startup; Node can discard unread
  // output when a short-lived child's pipes close.
  const streamPromise = readWorkerStream(child.stdout, sessionId, startedAt);
