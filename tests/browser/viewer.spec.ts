...
test('scene search filters Anchors and reveals the selected object', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write']);
  await page.setViewportSize({ width: 1600, height: 900 });
  await page.goto('/');
  await page.locator('input[type="file"]').first().setInputFiles({
    name: 'search.glb', mimeType: 'model/gltf-binary',
    buffer: triangleModel(true, false, [{ id: 'base', x: 0.25 }]),
  });
  const search = page.getByRole('textbox', { name: 'Search scene objects' });
  await page.getByRole('button', { name: 'Hide Other_triangle' }).click();
  await page.getByRole('combobox', { name: 'Filter scene objects' }).selectOption('hidden');
  await expect(page.getByRole('button', { name: 'Other_triangle', exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Test_triangle', exact: true })).toBeHidden();
  await page.getByRole('combobox', { name: 'Filter scene objects' }).selectOption('all');
  await search.fill('base');
  await expect(page.getByRole('button', { name: 'Test_triangle', exact: true })).toBeHidden();
  await page.getByRole('button', { name: 'base', exact: true }).click();
  await search.fill('not-found');
  await expect(page.getByText('No matching objects. Change the search or filter.')).toBeVisible();
  await page.getByRole('button', { name: 'Reveal selected', exact: true }).click();
  await expect(search).toHaveValue('');
  await page.getByRole('combobox', { name: 'Filter scene objects' }).selectOption('anchors');
  await expect(page.getByRole('button', { name: 'base', exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Test_triangle', exact: true })).toBeHidden();
  await page.getByRole('button', { name: 'base', exact: true }).dblclick();
  await page.getByRole('button', { name: 'Copy Anchor', exact: true }).click();
  expect(await page.evaluate(() => navigator.clipboard.readText())).toContain('World position');
  await page.getByRole('button', { name: 'Fit Anchor', exact: true }).click();
});

test('model replacement protects corrections but not view-only changes', async ({ page }) => {
  await page.setViewportSize({ width: 1600, height: 900 });
  await page.goto('/');
  ...
  const replacement = { name: 'replacement.glb', mimeType: 'model/gltf-binary', buffer: triangleModel() };
  await page.locator('input[type="file"]').first().setInputFiles(replacement);
  await expect(page.getByRole('alertdialog')).toContainText('Unsaved model changes');
  await page.getByRole('button', { name: 'Keep editing', exact: true }).click();
  await expect(page.getByRole('button', { name: /Open another model.*test-triangle/ })).toBeVisible();
  await page.locator('input[type="file"]').first().setInputFiles(replacement);
  await page.getByRole('button', { name: 'Discard changes', exact: true }).click();
  await expect(page.getByRole('button', { name: /Open another model.*replacement/ })).toBeVisible();
  await page.keyboard.press('Escape');
  await page.getByRole('toolbar', { name: 'Viewer tools' }).getByRole('button', { name: 'Grid', exact: true }).click();
  await page.getByRole('toolbar', { name: 'Viewer tools' }).getByRole('button', { name: 'Lighting', exact: true }).click();
  await page.getByRole('button', { name: 'Studio', exact: true }).click();
  await openTestModel(page);
  await expect(page.getByRole('alertdialog')).toBeHidden();
});

test('failed local import offers a retry without dropping the loaded model', async ({ page }) => {
  await page.goto('/');
  await openTestModel(page);
  await page.locator('input[type="file"]').first().setInputFiles({ name: 'broken.glb', mimeType: 'model/gltf-binary', buffer: Buffer.from('broken') });
  const failure = page.getByRole('alert').filter({ has: page.getByRole('button', { name: 'Retry', exact: true }) });
...
test('Undo restores the saved checkpoint and partial exports keep unsaved protection', async ({ page }) => {
  await page.setViewportSize({ width: 1600, height: 900 });
  await page.goto('/');
  await openTestModel(page);
  ...
  await expect(page.getByRole('button', { name: /Open another model.*replacement/ })).toBeVisible();
  await expect(page.getByRole('alertdialog')).toBeHidden();
  await toolbar.getByRole('button', { name: 'Adjust model' }).click();
  await page.getByRole('spinbutton', { name: 'Known dimension value' }).fill('250');
  await page.getByRole('button', { name: 'Apply calibration' }).click();
  await toolbar.getByRole('button', { name: 'Export model' }).click();
  await page.getByRole('button', { name: 'Visible only', exact: true }).click();
  let download = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Export GLB', exact: true }).click();
  await download;
  const another = { ...replacement, name: 'another.glb' };
  await page.locator('input[type="file"]').first().setInputFiles(another);
  ...
  await download;
  await page.locator('input[type="file"]').first().setInputFiles(another);
  await expect(page.getByRole('button', { name: /Open another model.*another/ })).toBeVisible();
  await expect(page.getByRole('alertdialog')).toBeHidden();
});

for (const viewport of [{ width: 320, height: 568 }, { width: 390, height: 844 }, { width: 844, height: 390 }]) {
  test(`export keeps its action outside the scroll body at ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    await page.setViewportSize(viewport);
    await page.goto('/');
    await openTestModel(page);
    await page.getByRole('toolbar', { name: 'Mobile viewer tools' }).getByRole('button', { name: 'More tools' }).click();
    await page.getByRole('button', { name: 'Export model', exact: true }).click();
    const dialog = page.getByRole('dialog', { name: 'Export', exact: true });
    const body = dialog.getByTestId('responsive-panel-scroll');
    const action = dialog.getByRole('button', { name: 'Export GLB', exact: true });
    ...
    expect(await body.getByRole('button', { name: 'Export GLB', exact: true }).count()).toBe(0);
    ...
    await page.screenshot({ path: testInfo.outputPath('compact-export.png') });
  });
...
test('empty viewer presents one clear local-file action', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Core source' })).toHaveAttribute('href', /https:\/\/github\.com\/SinaGh121\/kea3d-core\/releases\/tag\/v0\.1\.\d+$/);
  await expect(page.getByText('The Core source link provides source for this version. Third-party components retain their own licenses.')).toBeVisible();
...
test('unsupported BLEND files show GLB guidance', async ({ page }) => {
  await page.goto('/');
  await page.locator('input[type="file"]').first().setInputFiles({
  ...
  await expect(page.getByText(/Blender \(\.blend\) files are not supported/)).toBeVisible();
  await expect(page.getByText(/detached ArrayBuffer/)).toHaveCount(0);
});

for (const size of [
  { width: 390, height: 844, orthographic: false },
  { width: 1600, height: 900, orthographic: false },
  { width: 390, height: 844, orthographic: true },
  { width: 1600, height: 900, orthographic: true },
]) {
  test(`view selector fits and cancels back to the previous camera at ${size.width}, orthographic=${size.orthographic}`, async ({ page }) => {
    await page.setViewportSize(size);
    await page.goto('/');
    await openTestModel(page);
    if (size.orthographic) await page.keyboard.press('p');
    ...
    const camera = () => page.evaluate(() => JSON.parse(localStorage.getItem('kea3d.settings.v1') ?? '{}').viewer?.lastCamera);
    await viewport.hover();
    for (let index = 0; index < 8; index += 1) await page.mouse.wheel(0, -500);
    await page.waitForTimeout(500);
    const before = await camera();
    const distance = (state: typeof before) => Math.hypot(...state.position.map((value: number, index: number) => value - state.target[index]));
    for (const cancel of ['escape', 'toggle', 'outside']) {
      await selector.click();
      await expect(selector).toHaveAttribute('aria-pressed', 'true');
      await expect.poll(async () => distance(await camera())).toBeGreaterThan(distance(before) * 1.25);
      const bounds = (await viewport.boundingBox())!;
      if (cancel === 'escape') {
        await page.mouse.move(bounds.x + bounds.width * 0.5, bounds.y + bounds.height * 0.5);
        await page.mouse.down();
        await page.mouse.move(bounds.x + bounds.width * 0.6, bounds.y + bounds.height * 0.55, { steps: 10 });
        await page.mouse.up();
        await page.keyboard.press('Escape');
      } else if (cancel === 'toggle') await selector.click();
      else await page.mouse.click(bounds.x + 5, bounds.y + bounds.height * 0.5);
      await expect(selector).toHaveAttribute('aria-pressed', 'false');
      await expect.poll(async () => {
        const after = await camera();
        return Math.max(...['position', 'target', 'up'].flatMap((key) => after[key].map((value: number, index: number) => Math.abs(value - before[key][index]))));
      }).toBeLessThan(0.0001);
      expect((await camera()).projection).toBe(before.projection);
      if (size.orthographic) expect((await camera()).orthographicHeight).toBeCloseTo(before.orthographicHeight, 5);
    }
  });
}
